...
def convert_to_utc_austria(timeseries_data: pd.DataFrame) -> pd.DataFrame:
    ...
    if not time_in_UTC:
        ...
        print("Time stamps converted to UTC.")
    else:
        print("Time stamps are already UTC.")
...
def prepare_timeseries_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    timeseries_data = timeseries_data.resample("10T").first()
    ...
    print(f"number of invalid entries: {number_of_invalids}")
    print(f"data rangeing from {start} to {end}")
